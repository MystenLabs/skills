...
4. Does the query **join across entities** (e.g., "all NFTs owned by X minted after date Y with field Z > N" needs object + event + time data in one result)? → **GraphQL RPC** (`graphql.md`). Single-entity filtered queries over transactions or events (by sender, function, or event type) are now covered by gRPC `client.core.listTransactions` / `listEvents`.
...
| Show recent transactions by an address | gRPC (`client.core.listTransactions({ filter: { sender }, order: 'descending' })`) | Filtered + cursor-paginated on gRPC |
| Wallet transaction history across time | GraphQL OR gRPC (`listTransactions` with cursors) | gRPC for simple sender/function filters; GraphQL if joining with object changes |
...
| Query patterns | Flexible, nested, relational queries joining transactions, objects, events, balances in one request | Point lookups, transaction execution, simulation, single-dimension filtered queries (`listTransactions` / `listEvents` by sender, function, or event type), workflows modeled around protobuf messages |
| Historical access | Can use configured Archival Service for supported historical point lookups; supports filtered pagination over historical txs and events | Supports filtered pagination over historical txs and events within full-node retention; does not implicitly fall back to archival — clients need a separate Archival Service endpoint for data beyond full-node retention |
| Streaming | No subscription support yet | gRPC subscriptions for live checkpoint/transaction/event streaming |
