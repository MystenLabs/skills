...
2. **Don't fetch a separate `Coin<SUI>` via RPC as the fix for gas-coin misuse.** For transaction building, prefer gas smashing (`/develop/transaction-payment/gas-smashing`) or address-balance gas payments (`/onchain-finance/asset-custody/address-balances/using-address-balances`) over manual coin selection. Splitting off `tx.gas` covers the common case of needing an owned SUI coin.
...
5. **Use `Transaction.fromKind(kindBytes)` for sponsored flows.** Build in app with `tx.build({ client, onlyTransactionKind: true })`, send the kind-only bytes to the sponsor service, rehydrate there with `fromKind`, then `setSender`, `setGasOwner`, `setGasPayment`.
   - **Both parties must sign the entire `TransactionData`, including `GasData`.** This prevents a third party, such as a malicious full node, from intercepting partially signed data and changing the gas payment. A signature over only the transaction kind is not enough.
   - **Submit the fully-signed transaction directly to a full node** — the user (or either party) should do this rather than returning it to the sponsor to broadcast. A sponsor that holds the signed transaction can censor it by never submitting it; to avoid this risk, submit transactions directly to a full node.
...
7. **Shared objects are mutable and deletable — not immutable.** They can never be transferred or frozen, but a shared object *can* be taken by value in the one special case where the function that takes it **deletes** it. To mutate or consume a shared object (e.g. a pool), pass it as `mutable: true`; an input marked `mutable: false` is read-only and cannot be mutated or consumed.
